...
1. **Подготовка датасета**  
   Генерация и курирование диалогов через `data/generate_dialogs_dataset.py`.

2. **Тонкая настройка модели**  
   Настройка с использованием LoRA через `src/train_lora_cpu.py`.

3. **Тестирование и валидация**  
   Проверка качества ответов и корректности генерации кода.

4. **Развертывание через FastAPI**  
   Запуск сервера модели через `src/server.py`.

5. **Доступ через веб-интерфейс**  
   Работа с моделью через `static/App.html` и тестовую страницу.
